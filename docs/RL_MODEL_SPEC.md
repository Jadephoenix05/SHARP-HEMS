...
### ⚠ The ceiling is NOT what I first calculated — corrected

An earlier version of this section claimed a hard ceiling of 0.4911 on
`random_binary` rows and about 0.83 pooled, on the grounds that the policy draws
its action from fresh noise:

```python
draw = rng_local.random(n)                      # a draw, per device, per step
choice = np.where(draw < 0.34, 0, np.where(draw < 0.67, 1, 2))
```

**That calculation was wrong, and the error was found by Pranitha.** Her
independently trained model scored **0.694** on `random_binary` — above a number
I had called a hard cap — and rather than accept it she traced the cause. She
ruled out the shield and the human override (they alter under 0.01 % of any
policy's logged actions) and ruled out a state/action row-alignment bug, then
found the real reason: for many devices the logged action is 80–99 % persistent
across all 96 steps of a day.

Verified here on validation `random_binary` rows, 1,734 device-episodes:

| | |
|---|---|
| Median modal-action persistence across a day | **0.833** |
| Devices ≥ 80 % persistent | **77.9 %** |
| Devices ≥ 99 % persistent | 3.2 % |

Under fresh i.i.d. draws that figure would sit near 1/3. It does not, because
minimum-run and cycle constraints apply even under the "random" behaviour
policy. **That persistence is real structure a state-reading model can learn**,
through `current_on` and `elapsed_state_steps`, so the enumerated ceiling is a
**simplified lower bound, not a hard cap**.

The practical consequence: there is more headroom than this document previously
claimed, and it sits specifically in `random_binary`. Chasing it means learning
the persistence, not running more updates — Pranitha reached a better figure
there with 5,000 conservative updates against our 15,000.

The general lesson is the one worth keeping: **a ceiling derived from how the
data was supposed to be generated is not a measurement.** Check it against the
data before quoting it as a limit.
