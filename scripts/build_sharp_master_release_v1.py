...
ITEMS = [
    ...
    ('rl_transitions/scenario_pv/rl_transitions.parquet',
     'data/processed/sharp_rl_scenario_pv_v1/rl_transitions.parquet',
     'rl_transitions', 'Labelled 2 kW rooftop-PV scenario that exercises sink-aware shedding. NOT an AP ownership rate.', True),
    ('rl_transitions/scenario_pv/transition_validation.json',
     'data/processed/sharp_rl_scenario_pv_v1/transition_validation.json',
     'rl_transitions', 'PV scenario report and its scenario warning.', True),

    ('validation/e7_iawe_heldout_v1.json',
     'reports/e7_iawe_heldout_v1.json',
     'validation', 'E7: comparison against the iAWE New Delhi household, mains channels held out.', True),
    ('validation/aggregate_load_vs_emarc_v1.json',
     'reports/aggregate_load_vs_emarc_v1.json',
     'validation', 'External realism: simulated load against eMARC metered Indian households.', True),
    ('validation/aggregate_load_profile_vs_emarc_v1.csv',
     'reports/aggregate_load_profile_vs_emarc_v1.csv',
     'validation', 'Per-slot diurnal profile, simulated against metered.', True),
    ('validation/spec_validation_v1.json',
     'reports/spec_validation_v1.json',
     'validation', 'Every requirement in the idea book and source-of-truth guide, checked.', True),
    ('validation/unmodelled_background_load_v1.json',
     'reports/unmodelled_background_load_v1.json',
     'validation', 'How each household background load was calibrated to its own bill.', True),
    ('validation/household_billing_position_v1.json',
     'reports/household_billing_position_v1.json',
     'validation', 'Tariff-slab position, and why bill inversion is capped by connection capacity.', True),

    ('simulator_inputs/appliance_clock_profiles.csv',
     'data/processed/tus_appliance_clock_v1/appliance_clock_profiles.csv',
     'simulator_inputs', 'Appliance clock timing derived from TUS 2024 activity diaries.', True),
    ('simulator_inputs/household_background_load_v1.parquet',
     'data/processed/appliance_inputs_v1/household_background_load_v1.parquet',
     'simulator_inputs', 'Per-household unmodelled background load. Not controllable.', True),
